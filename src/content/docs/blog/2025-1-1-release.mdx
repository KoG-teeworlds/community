...
The Power Probability Matrix explains how likely it is that a player finish will trigger a Custom Random Map Event.
...
### <Badge text="New" variant="note" /> `/report` command

You are now able to report a player without opening a ticket on discord. To use this command, you can enter `/report <playername> <reason>`.
You can only report players connected to the current server.
...
- Merge upstream DDNet 18.8.2. [DDNet-Release 18.8.2](https://github.com/ddnet/ddnet/releases/tag/18.8.2)
- `PlayerFinishEvent` and `TeamFinishEvent` now unified into a `TeamFinishEvent`, to check if its a solo run you can rely on the length of the player list.
